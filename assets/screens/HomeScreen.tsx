...
// Revert to a simpler definition for AnimatedFlatList
...
const HomeScreen = () => {
    ...
    React.useEffect(() => {
        const fetchEvents = async () => {
            try {
                ...
                setRefreshing(false);
                // Optional: Cache fetched events
                AsyncStorage.setItem('cachedEvents', JSON.stringify(response.data));
            } catch (err) {
                console.error('Error fetching events:', err);
                setRefreshing(false);
                // Load cached events if fetching fails
                loadCachedEvents();
            }
        };

        if (!isOffline) { // Only fetch if online
            fetchEvents();
        } else {
            // If offline on mount, try loading cached events immediately
            loadCachedEvents();
        }
    }, [isOffline]);
    ...
    const filteredEvents: EventItem[] = (isOffline ? cachedEvents : cachedEvents).filter(
        (event: EventItem) =>
